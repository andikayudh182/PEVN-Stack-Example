...
module.exports = {
  list(req, res) {
    return Lecturer
      ...
      .then((lecturers) => res.status(200).send({
  ...
  addWithCourse(req, res) {
    return Lecturer
      ...
      .then((lecturer) => res.status(201).send({
        message:`add new lecturer with course success`,
        data : lecturer
      }))
      .catch((error) => res.status(400).send(error));
  },
      ...
      .then(lecturer => {
        ...
        return lecturer
          ...
          .then(() => res.status(200).send({
            message:`update lecturer by id${req.params.id} success` 
          }))
          .catch((error) => res.status(400).send(error));
      })
      .catch((error) => res.status(400).send(error));
      ...
      .then(lecturer => {
        ...
        return lecturer
          ...
          .then(() => res.status(204).send({
            message: `Delete lecturer with id ${req.params.id} success`,
          }))
          .catch((error) => res.status(400).send(error));
      })
      .catch((error) => res.status(400).send(error));
