...
module.exports = {
  list(req, res) {
    return Course
      ...
      .then((courses) => res.status(200).send({
    ...
    return Course
      ...
      .then((course) => {
        ...
        return res.status(200).send({
            message:`get data coruses by id ${req.params.id}`,
            data : course
        });
      })
      .catch((error) => res.status(400).send(error));
      ...
      .then(course => {
        ...
        return course
          ...
          .then(() => res.status(200).send({
            message:`update course by id ${req.params.id} success`,
            data : course
          }))
          .catch((error) => res.status(400).send(error));
      })
      ...
      .then(course => {
        ...
        return course
          ...
          .then(() => res.status(204).send({
            message: `Delete course with id ${req.params.id} success`,
          }))
          .catch((error) => res.status(400).send(error));
      })
      .catch((error) => res.status(400).send(error));
